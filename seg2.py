...
parser.add_argument('--displaymode',default='png', choices=['disabled', 'pdf', 'png', 'screen'], help='whether to show the segmentation result (screen), to write it to a file (pdf, png), or to skip it (disabled)')
...
np.random.seed(1)
for index,imgfn in enumerate(imagnames):
    image = imread(imgfn)
    print "{}/{} {}: {}".format( index+1, len(imagnames), imgfn, (image.shape[1], image.shape[0]) )
